...
class CustomUser(AbstractBaseUser, MPTTModel, PermissionsMixin):
    ...
    username = models.SlugField(max_length=55, db_index=True, unique=True, verbose_name=_('имя пользователя'),
    ...
    personal_number = models.CharField(_('ИНН или персональный номер'), max_length=17, blank=True, null=True,
                                       db_index=True)
    document = models.FileField(_('документ'), upload_to='documents/', blank=True,
    ...
    bank_correspondent_account = models.CharField(_('Корреспондентский счет банка'), max_length=55, blank=True,
                                                  null=True)
